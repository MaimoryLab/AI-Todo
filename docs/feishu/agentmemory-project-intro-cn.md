...
这张图补充解释 Viewer 内部的闭环：浏览器对话、Agent 会话和手动添加内容都会先进入待审阅队列，不直接写入长期记忆。用户确认时把候选改成具体事实，删掉导航文字、临时链接和噪音，再分流到记忆库、经验库、行动卡片或 Skill 草稿。沉淀后的记忆和经验会回到下一次对话，让 Agent 更快接上项目上下文。

| 工作台环节 | 用户看见什么 | 为什么重要 |
| --- | --- | --- |
| 入口 | 浏览器对话、Agent 会话、手动添加 | 所有线索先进入同一个审阅入口 |
| 待审阅队列 | 候选内容、来源、时间 | 自动整理不等于自动乱存 |
| 人工确认 | 改标题、正文、类型、保存范围 | 把网页片段改成具体事实 |
| 记忆库 | 身份、偏好、项目背景和来源 | 下次 Agent 可以更快接上上下文 |
| 经验库 | 排布原则、交互原则、协作习惯 | 把一次对话沉淀成下次能复用的经验 |
| 行动卡片 | 待跟进、正在推进、需要处理、已完成 | 用自然语言承接未完成事项 |
