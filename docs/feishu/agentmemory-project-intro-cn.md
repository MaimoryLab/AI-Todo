...
| 浏览器插件 | MVP 已完成，能把网页候选送到待审阅队列，并提供同步侧栏和 AI 输入框记忆提示 |
...
| P0 | 活动页加载兜底 | 活动页不应长期停在加载态 |
| P1 | 精准 AI 对话抽取器 | ChatGPT、Claude、Gemini、Perplexity 应有不同采集方式 |
| P1 | AI 输入框注入精调 | 不同网页的输入框 DOM 和事件机制不一样，需要逐站优化 |
